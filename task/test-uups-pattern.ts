...
task("test-uups-pattern", "Test the UUPS pattern deployment")
    .setAction(async (taskArgs, hre) => {
        ...
        const { address: deployUUPSExampleImpl, hash: deployUUPSExampleImplHash } = await deployer.deployContract({
            shardId: 1,
        ...
        console.log("Tx Hash for deployment of UUPS Example Implementation contract:", deployUUPSExampleImplHash);
        ...
        const { address: deployProxy, hash: deployProxyHash } = await deployer.deployContract({
            shardId: 1,
        ...
        console.log("Tx Hash for deployment of UUPS Proxy contract:", deployProxyHash);
        console.log("Deployed Proxy Address:", deployProxy);

    console.log("Checking if getImplementation exists in ABI...");
    const hasGetImplementation = ERC1967Proxy.abi.some((entry: any) => entry.name === "getImplementation");
...
if (!hasGetImplementation) {
    ...
    const { address: newUUPSExampleImpl, hash: newUUPSExampleImplHash } = await deployer.deployContract({
        shardId: 1,
    ...
    console.log("Tx Hash for deployment of UUPS Example V2 Implementation contract:", newUUPSExampleImplHash);
    ...
    console.log("Value after reinitialization:", valueAfterUpgrade as unknown as number);
    ...
    console.log("User Balance after deposit:", userBalanceDecoded as unknown as number);
